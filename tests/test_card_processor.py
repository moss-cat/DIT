...
=======
    ...
    def test_load_default_deck_success(self, processor, sample_dataframe):
        """Test loading a default deck successfully."""
        with patch('pathlib.Path.exists', return_value=True):
            with patch.object(processor, '_load_and_validate_csv', return_value=sample_dataframe) as mock_load:
                df = processor.load_default_deck('python')
    ...
    def test_load_custom_deck_success(self, processor, sample_csv_content, sample_dataframe):
        """Test loading a custom deck successfully."""
        with patch.object(processor, '_validate_dataframe', return_value=sample_dataframe) as mock_validate:
            df = processor.load_custom_deck(sample_csv_content)
